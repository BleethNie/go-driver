...
type serverClient struct {
	...
	slave            *Server
	client           Client
	clientTCPHandler *TCPClientHandler
}
...
func serverClientSetup() *serverClient {
	...
	// Client
	setup.clientTCPHandler = NewTCPClientHandler(addr)
	...
	setup.client = NewClient(setup.clientTCPHandler)
...
func Example() {
	...
	// Connect a client.
	handler := NewTCPClientHandler("localhost:1502")
	err = handler.Connect()
	if err != nil {
		log.Printf("%v\n", err)
		return
...
func ExampleServer_RegisterFunctionHandler() {
	...
	// Connect a client.
	handler := NewTCPClientHandler("localhost:4321")
	err = handler.Connect()
	if err != nil {
		log.Printf("%v\n", err)
		return
